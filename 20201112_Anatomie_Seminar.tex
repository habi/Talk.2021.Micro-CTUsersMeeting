...
\begin{frame}{Quantitative data}
	\begin{itemize}
		...
		\item All automated as much as possible with Jupyter~\cite{Kluyver2016}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{tikzpicture}[remember picture,overlay]%
		\node at (current page.center){%
			\mode<beamer>{\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./movies/tooth045/full/image}{0000}{0179}}%
			\mode<handout>{\includegraphics[width=\paperwidth]{./movies/tooth045/full/image0000}}%
			};%
	\end{tikzpicture}%
\end{frame}
...
\begin{frame}{How?}
	\begin{columns}
		...
		\begin{column}{0.49\linewidth}
			...
			\only<2|handout:0>{%
				fishvideo%\animategraphics[loop,autoplay,width=\linewidth,every=\everyframe]{24}{./movies/training/fishvideo_0}{001}{250}%
				}%
			\only<3|handout:2>{\centering\Huge\faTooth:\xspace\faRuler\xspace\&\xspace\faWeight}
			...
			\only<6|handout:5>{%
				LOG FILE%\lstinputlisting[linerange={2-5,17-17,19-23,29-31,34-34,38-40,51-51,58-59}]{./logfiles/Control05/proj/Control05.log}%
				}
			CTAN\includegraphics<7|handout:6>[height=\imheight]{./images/CTAn}%
			\only<8|handout:7>{%
				\mode<beamer>{%
...
\begin{frame}{Gill complexity}
	...
	\only<4|handout:4>{%
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center){%
				GILLS%\mode<beamer>{\animategraphics[palindrome,autoplay,width=\paperwidth,every=\everyframe]{24}{./movies/complexity/control03_thresholded_0}{030}{256}}%
				GILLS%\mode<handout>{\includegraphics[width=\paperwidth]{./movies/complexity/control03_thresholded_0068}}%
				};%
		\end{tikzpicture}%
		}%
...
\begin{frame}{Thanks!}
	\begin{itemize}
		...
		\item<2-> \href{https://twitter.com/jackiantonovich/status/1195699076056178691}{What questions do you have for me} and \href{https://twitter.com/ElaineLuther/status/1195708328854331392}{what can I clarify for you}?
	\end{itemize}
	\only<3|handout:0>{%
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center){%
...
%- How many images did we record
%- How much data is there, how much is saved 
%- How much did we save
%- Manual process -> lots of human intervention -> how long would it take
%- Automated process -> *Fully* reproducible -> Binder
%- Planned analysis for Andrea -> EDT
